...
- Optional WASI preview2 execution mode via `wasi_preview2 = true`.
  - Component Model execution behind `component = true` (experimental).
- Optional execution limits via `limits`: fuel, wall_time, max_output_bytes, memory.
- Thin OCI wrapper: `shar oci run <bundle_dir>` delegates to `shar run`.
- Signing + verification: `shar sign`, `shar verify`, `shar run --require-signature`.
